...
#### 7.5 홈 내비게이터
- **기능**: 홈 화면에서 앱의 모든 주요 기능과 진입 경로를 한눈에 안내
- **구성 요소**:
  - 기능 카테고리 카드 (추모관 관리, 공감 공유, AI 기능, 혜택/설정 등)
  - 각 기능 설명과 대표 아이콘
  - "바로 가기" 버튼 또는 단계 안내
- **동작 방식**:
  - 카드 터치 시 해당 화면으로 직접 이동
  - 선행 조건이 필요한 경우 안내 다이얼로그 표시
  - 첫 방문 시 튜토리얼 모드로 연동 가능
- **표시 위치**: 홈 화면 중간 (앱 기능 안내 섹션 아래), 또는 별도 "전체 기능 보기" 버튼으로 진입
- **추가 옵션**:
  - UI 프리뷰 썸네일을 활용한 화면 미리보기 제공
  - 최근 사용한 기능 상단 고정, 즐겨찾기 기능
  - 접근성 모드를 위한 텍스트 리스트 뷰 제공
